  ...
  <pop-up class="relative flex flex-row items-center w-fit justify-center w-full z-20 h-full mx-auto" :disable="disable"
    ...
    <div class="rtl w-[30vw] mx-auto bg-slate-800 rounded-lg p-4 mt-[10vh]">
      ...
      <div class="mb-6">
        ...
        <select id="parent" v-model="userData.parent"
                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
        >
      ...
      <plux-button title="ثبت دسته بندی" type="square" color="none"
  ...
  if (errors.value.length === 0) {
    ...
    if (userData.value.parent === 'null') {
      ...
      payload = {
        title: userData.value.title,
        description: userData.value.description,
        parent_id: userData.value.parent,

      }
    }
    store.dispatch('appendCategories',payload).then(() => store.dispatch('generateCategories').then(()=>userData.value={
      title: '',
      description: '',
      parent: 'null'
    }))
